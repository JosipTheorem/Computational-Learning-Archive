...
sim_folder = '/storage/JOSIPDUJMENOVIC_STORAGE/graphene/bands_4x4/polaronXY'
...
Efermi = -4.5379     # Fermi energy from the 2x2 (unfolded) calculation
...
for i, Emin in enumerate(E_min):
    ...
    ax_dos = divider.append_axes("right", size="40%", pad=0.05, sharey=ax)
...
# Add the z-polaron DOS
    file_vacancy = "/storage/JOSIPDUJMENOVIC_STORAGE/graphene/bands_4x4/TEST1/stari/stari_xy_BZ/bandstructure_unfolded.txt"
    E_F_vac = -4.5450
    E_vac, w_vac = read_energies_weights(file_vacancy)
    E_vac -= E_F_vac
    dos_vac_z = lorentzian_dos(E_vac, w_vac, E_grid, eta, normalize=False)
    ax_dos.plot(dos_vac_z, E_grid, color='red', linestyle='--', label='xy_polaron')
